...
<div>
    ...
    <div x-show="showFilters" x-transition x-cloak class="mb-6 rounded-lg border border-slate-200 bg-white p-5">
        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
            ...
            {{-- -- DROPDOWN FILTER RT -- --}}
            <div>
                <label for="filterRt" class="block text-sm font-medium text-main">RT</label>
                <x-forms.select id="filterRt" wire:model.live="filterRt" class="mt-1">
                    <option value="">Semua RT</option>
                    @foreach($opsiRt as $rt)
                    <option value="{{ $rt }}">RT {{ $rt }}</option>
                    @endforeach
                </x-forms.select>
            </div>
            {{-- PENAMBAHAN: Filter Status Perkawinan --}}
            <div>
                <label for="filterStatusPerkawinan" class="block text-sm font-medium text-main">Status
                    Perkawinan</label>
                <x-forms.select id="filterStatusPerkawinan" wire:model.live="filterStatusPerkawinan" class="mt-1">
                    <option value="">Semua</option>
                    @foreach($opsiStatusPerkawinan as $key => $value)
                    <option value="{{ $key }}">{{ $value }}</option>
                    @endforeach
                </x-forms.select>
            </div>

            {{-- PENAMBAHAN: Filter Pendidikan --}}
            <div>
                <label for="filterPendidikan" class="block text-sm font-medium text-main">Pendidikan Terakhir</label>
                <x-forms.select id="filterPendidikan" wire:model.live="filterPendidikan" class="mt-1">
                    <option value="">Semua</option>
